		...
		<div class="row" style="z-index: 2;">
			...
			<div class="col s1">
				...
				<form method="get" action="{{ action('CartController@create') }}">
					<label for="start">Date de départ : </label> <input type="date"
						id="start" name="trip-start" value="2019-04-10" min="2019-04-10"
						max="2050-12-31"> <label>Choisissez le nombre de personnes : </label>
					<select class="browser-default" name="nbpers" id="nbpers">
						<option class="taille" value="">Plus on est de fous, plus on rit
							:)</option>
					...
					</select> <input id="prodId" name="prodId" type="hidden"
						value="{{ $product->id }}"> <input type="submit" class="basket"
						value="Je Pars en voyage zge">
				</form>
			</div>
			@if(isset($user) and $user->is_admin == 1)

			<form class="form-horizontal" method="POST"
				action="{{ route('product.destroy', $product->id) }}">
				{{ csrf_field() }} {{ method_field('DELETE') }} <a
